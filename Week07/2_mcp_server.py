...
# "youtube_agent_server"는 클라이언트가 인식하는 식별자로 어떤 이름도 상관없음
# "youtube_agent_server"라는 이름을 가진 도구 집합(toolset) MCP 서버를 생성하는 것이고, 
# 이후 @mcp.tool()로 등록된 유튜브 분석 함수들을 클라이언트에서 호출할 수 있도록 준비해주는 역할
...
# 함수 단위의 Tool로 MCP에서 사용할 수 있도록 선언하고, FastMCP 서버를 통해 외부에서 이 함수들을 호출 가능하게 구성
@mcp.tool()
